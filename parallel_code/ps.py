...
def distribute_MSAs_over_jobs(path_list, n_jobs, all_jobs_results_folder, max_n_sequences,
                              n_random_starting_trees,random_trees_training_size,random_trees_test_size, jobs_prefix,run_prefix,baseline_run_prefix, only_evaluate_lasso,exp_brlen, uni_brlen, opt_brlen):
    jobs_csv_path_list = []
        ...
        if not LOCAL_RUN:
            ...
            theproc = subprocess.Popen(
                [sys.executable, msa_code_location, "--job_ind", str(job_ind), "--curr_job_folder", curr_job_folder,
                 ...
                 str(random_trees_training_size),'--random_trees_test_size',str(random_trees_test_size),'--run_prefix',run_prefix,'--baseline_run_prefix',baseline_run_prefix] + only_evaluate_lasso_arg +exp_brlen_arg + uni_brlen_arg + opt_brlen_arg)
            theproc.communicate()
