...
def get_non_greedy_optimized_SPR_neighbours(curr_msa_stats, MSA_path, unique_trees_path, weights_file_path,lasso_intercept, curr_run_directory, n_cpus):
    ...
    res = {"spr_candidates_for_brlen_opt_file": spr_candidates_for_brlen_opt_file,"trees_ll_no_brlen" : ll_all_spr_candidates_corrected, "ll_spr_candidates_for_brlen_corrected": ll_spr_candidates_for_brlen_corrected,
...
def SPR_iteration(iteration_number, MSA_path, curr_msa_stats, starting_tree_object,
    ...
    spr_evaluation_data = get_non_greedy_optimized_SPR_neighbours(curr_msa_stats, MSA_path, unique_spr_neighbours_path, weights_file_path,lasso_intercept, curr_run_directory, n_cpus)
    ...
    if weights_file_path:
        ...
        best_ll, best_ll_index, best_tree_object, re_optimization_time = re_optimize_some_SPR_neighbours_no_weights(spr_evaluation_data["ll_spr_candidates_for_brlen_corrected"],top_x_true_trees,spr_evaluation_data["optimized_objects_spr_candidates_for_brlen"], curr_msa_stats, curr_run_directory)
        ...
        if curr_msa_stats["compute_all_true_ll"]:
            tree_eval_true_ll = get_true_ll_values(curr_msa_stats, unique_spr_neighbours_path,
                                                  curr_run_directory, opt_brlen = False)
            tree_opt_true_ll = get_true_ll_values(curr_msa_stats, spr_evaluation_data["spr_candidates_for_brlen_opt_file"],
    ...
    ll_comparison_df_brlen_eval = pd.DataFrame(
        {'full msa ll': tree_eval_true_ll, 'sampled msa ll': spr_evaluation_data["trees_ll_no_brlen"], 'iteration number': iteration_number}
    )
    ll_comparison_df_brlen_opt = pd.DataFrame(
        {'full msa ll': tree_opt_true_ll, 'sampled msa ll': spr_evaluation_data["ll_spr_candidates_for_brlen_corrected"],
         'iteration number': iteration_number}
    )
    logging.info(f"iteration {iteration_number}  best tree ll = {best_ll}")

    results_dict = {"best_tree_object":best_tree_object, "best_ll" : best_ll,"best_true_ll": best_true_ll, "ll_comparison_df_brlen_eval":ll_comparison_df_brlen_eval,"ll_comparison_df_brlen_opt":ll_comparison_df_brlen_opt
                    , "iteration_time": iteration_time, "iteration_time_brlen" : spr_evaluation_data["iteration_time_brlen"], "iteration_time_no_brlen":spr_evaluation_data["iteration_time_no_brlen"], "re_optimization_time":re_optimization_time, "n_neighbours": len(spr_evaluation_data["trees_ll_no_brlen"]) }
    return results_dict
...
def SPR_search(MSA_path, run_unique_name, curr_msa_stats, starting_tree_path, starting_tree_object,
    ...
    logging.info("Search starting tree ll = {} Search true starting tree ll = {}. Using only true starting tree values ".format(search_starting_tree_ll,
    ...
    while True:
        ...
        ll_comparison_df_brlen_eval = ll_comparison_df_brlen_eval.append(new_iteration_results["ll_comparison_df_brlen_eval"])
        ll_comparison_df_brlen_opt = ll_comparison_df_brlen_opt.append(new_iteration_results["ll_comparison_df_brlen_opt"])
    ...
    if (full_run):
        ...
        for i,lasso_configuration in enumerate(lasso_configurations):
            ...
            curr_phase_results_print = {k: curr_phase_param_dict [k] for k in curr_phase_param_dict.keys() if
                                     k not in ["ll_comparison_opt","ll_comparison_eval", "actual_search_training_df"]
                                     }
            ...
            curr_phase_param_dict["ll_comparison_opt"].to_csv(os.path.join( sub_curr_run_directory ,f"{i}_phase_ll_comparison_opt.csv"))
        ...
        final_optimized_print = {k: final_phase_param_dict[k] for k in final_phase_param_dict.keys()
                                 if
                                 k not in ["ll_comparison_eval","ll_comparison_opt", "actual_search_training_df"]
                                 }
