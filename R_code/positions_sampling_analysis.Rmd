...
g1<-ggplot(example_MSA_data_lasso, aes(x=as.factor(actucal_training_size),fill=brlen_generator, y=lasso_test_spearmanr))+ 
    ...
    xlab("Training size")+ylab("Spearman rho")+theme(axis.title=element_text(size=10))+guides(fill=guide_legend(title="Branch length distribution"))+ggtitle("Percentage of positions chosen by Lasso")+ theme(plot.title = element_text(hjust = 0.5)) 
...
#title="Percentage of positions chosen by Lasso vs. training size"
 ...
 g2<-ggplot(example_MSA_data_lasso, aes(x=as.factor(actucal_training_size),fill=brlen_generator, y=sample_pct))+ labs(color = "Branch length distribution")+
   ...
   xlab("Training size")+ylab("% Positions")+ theme(axis.title=element_text(size=10),)+guides(fill=guide_legend(title="Branch length distribution"))+ggtitle("Percentage of positions chosen by Lasso")+scale_y_continuous(labels = scales::percent)+ theme(plot.title = element_text(hjust = 0.5)) 

ggarrange(g0, g1,g2, hjust=-1,vjust=0, heights=c(2,2),common.legend=TRUE, 
          labels = c("A", "B","C"),
          ncol = 1, nrow =3)
...
ggarrange(g0, g1,g2,hjust=-1,vjust=0,common.legend=TRUE,align="h",
          labels = c("A", "B","C"),
          ncol = 1, nrow =3)
...
ggarrange(g0, g1,g2, hjust=-2,vjust=1, common.legend=TRUE,
          labels = c("A", "B","C"),
          ncol = 1, nrow =3)
...
summary(glht(model.uniform, linfct=mcp(training_size_factor = "Tukey")), test = adjusted(type = "bonferroni"))
...
summary(glht(model.exponential, linfct=mcp(training_size_factor = "Tukey")), test = adjusted(type = "bonferroni"))
```
