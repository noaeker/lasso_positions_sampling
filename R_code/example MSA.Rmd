...
```{r}
ll_comparison_data = read_csv("/Users/noa/Workspace/lasso_positions_sampling_results/ll_comparison_df_800_0.1_borowik.csv")
...
plot_a_top<-ll_comparison_data %>% group_by(`iteration number`) %>%slice_max(order_by =`full msa ll`, n = 30) %>% summarise(spearman_r = cor(`full msa ll`, `sampled msa ll`, method = 'spearman')) %>% ggplot(aes(x=as.factor(`iteration number`), y= spearman_r)) + geom_point(size=3) +geom_line(group = 1 ) + xlab("iteration_number") + ggtitle ("Spearman R of top  neighbours")

plot_b_top<- ll_comparison_data %>% group_by(`iteration number`) %>%slice_max(order_by =`full msa ll`, n = 30)  %>% summarise(full_data_variance = sqrt(var(`full msa ll`))) %>%  ggplot(aes (x=as.factor(`iteration number`), y= `full_data_variance`)) + geom_point() +geom_line(group = 1) + ggtitle("True LL variance of top  neighbours") + xlab("iteration_number")

plot_c_top<- ll_comparison_data %>% group_by(`iteration number`) %>%slice_max(order_by =`sampled msa ll`, n = 30)  %>% summarise(sampled_data_variance = sqrt(var(`sampled msa ll`))) %>%  ggplot(aes (x=as.factor(`iteration number`), y= `sampled_data_variance`)) + geom_point() +geom_line(group = 1) + ggtitle("Sampled LL variance of top  neighbours") + xlab("iteration_number")
...
approximated_rank_data %>% arrange(approx_rank)
