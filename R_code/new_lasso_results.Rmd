...
```{r}
spr_data %>% distinct(db,dataset_id, n_seq) %>% group_by(db, n_seq)%>% summarize(n=n())
```

Summaries

```{r}
summary(spr_data$lasso_running_time/60)
```
...
```{r}
example_msa_data<- spr_data %>% filter (relative_path=="Selectome/Euteleostomi/ENSGT00680000099951", actucal_training_size==800)

example_msa_test_ll<- read_csv("/Users/noa/Workspace/lasso_positions_sampling_results/test_sitelh_df_prediction_51.csv")
example_msa_training_ll<- read_csv("/Users/noa/Workspace/lasso_positions_sampling_results/training_sitelh_df_prediction_51.csv")
...
unexplained_variance_data<- spr_data %>% filter (relative_path=="Selectome/Euteleostomi/ENSGT00680000099951")  %>% select (sample_pct,actucal_training_size,`lasso_test_R^2`, lasso_test_spearmanr) %>% mutate (unexplained_var =1-`lasso_test_R^2` )

unexplained_variance_data

unexplained_variance_data %>% ggplot(aes(x=sample_pct,y=unexplained_var, group = as.factor(actucal_training_size), colour = as.factor(actucal_training_size))) +geom_line() + geom_point()+ scale_x_continuous(labels = scales::percent) +labs(color="Training size",x="Sample percentage (t)",y="Unexplained variance (%)", title="Percentage of unexplained variance on test set") +  scale_y_continuous(labels = scales::percent, limits = c(0,0.025))+theme_classic()
...
spr_data %>% filter (relative_path=="Selectome/Euteleostomi/ENSGT00680000099951") %>% select ( actucal_training_size,actucal_sample_pct,  delta_ll_first_phase)

spr_data %>% filter (relative_path=="Selectome/Euteleostomi/ENSGT00680000099951") %>% ggplot(aes(x=sample_pct,y=delta_ll_first_phase, group = as.factor(actucal_training_size), colour = as.factor(actucal_training_size))) +geom_line() + geom_point()+ scale_x_continuous(labels = scales::percent) +labs(color="Training size",x="Sample percentage (t)",y="log-likelihood difference", title="log-likelihood differences") +theme_classic()
...
full_data   %>% group_by(actucal_training_size, actucal_sample_pct) %>% summarize(median_r_2 = median(valid_test_r2)) %>% ggplot(aes(x=actucal_sample_pct,y=1-median_r_2, group = as.factor(actucal_training_size), colour = as.factor(actucal_training_size))) +geom_line() + geom_point()+ scale_x_continuous(labels = scales::percent) +labs(color="Training size",x="Sample percentage (t)",y="Unexplained variance (%)", title="Median percentage of unexplained variance on test set") + scale_y_continuous(labels = scales::percent, limits = c(0,0.025))+theme_classic() 

full_data %>% filter (actucal_training_size==800 & actucal_sample_pct==0.2) %>% summarize(median_r_2 = median(valid_test_r2))

```
```{r}
full_data   %>% group_by(actucal_training_size, actucal_sample_pct) %>% summarize(median_delta_ll = median(valid_delta_ll_first_phase )) %>% ggplot(aes(x=actucal_sample_pct,y=median_delta_ll , group = as.factor(actucal_training_size), colour = as.factor(actucal_training_size))) +geom_line() + geom_point()+ scale_x_continuous(labels = scales::percent) +labs(color="Training size",x="Sample percentage (t)",y="log-likelihood differences", title="Median log-likelihood differences") +theme_classic() 
...
full_data   %>% group_by(actucal_training_size, actucal_sample_pct) %>% summarize(pct_identical = sum(valid_rf_first_phase==0)/sum(valid_rf_first_phase==0 | valid_rf_first_phase>0),median_delta_ll= median(valid_delta_ll_first_phase), max_spr_dist = max(valid_lasso_SPR_second_phase_spr_moves), median_rf = median(valid_rf_first_phase), median_run_time_factor = median(run_time_factor))
