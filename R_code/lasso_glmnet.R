...
n_loci = ncol(X)
pcts = c(0.005,0.01,0.05,0.1,0.15)
counts = pcts*n_loci
relevant_non_zero_cnts<-coefs_cnt %>% crossing(pcts) %>%  mutate(counts = pcts*n_loci,diff = (non_zero_cnt-counts)) %>% filter (diff>=0) %>% group_by(counts, pcts) %>% filter (diff ==min(diff)) %>% pull(non_zero_cnt) 
