...
lasso_path = glmnet(x = X, y = y, nlambda = 100, lambda.min.ratio = 0.0000001, alpha =1,lower.limits=0)
...
counts = pcts*n_loci
relevant_non_zero_cnts<-coefs_cnt %>% crossing(pcts) %>%  mutate(counts = pcts*n_loci,diff = (counts-non_zero_cnt)) %>% filter (diff>=0) %>% group_by(counts, pcts) %>% filter (diff ==min(diff)) %>% pull(non_zero_cnt) 
