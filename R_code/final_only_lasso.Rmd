...
example_msa_test_ll<- read_csv("/Users/noa/Workspace/lasso_positions_sampling_results/NagyA1_test.csv")
...
```{r}
t30_2000<-new_format_lasso_data %>% filter (actual_training_size==2000, actual_n_seq==30, actual_n_loci==80000) %>% arrange (dataset_name) %>%pull (`lasso_test_R^2`)
t30_4000<-new_format_lasso_data %>% filter (actual_training_size==4000, actual_n_seq==30, actual_n_loci==80000) %>% arrange (dataset_name) %>% pull (`lasso_test_R^2`)
t.test(t30_2000,t30_4000, paired = TRUE)
```
...
```
```{r}
new_format_lasso_data %>% filter (n_seq==30, n_loci == 80000) %>%  distinct (dataset_name,constant_sites_pct, gap_pct)
```
